 ...
 */
import { createInitialState, applyAction, advanceToNextRound, } from '../game/stateMachine.js';
    ...
    while (roundCount < TARGET_ROUNDS && state.phase !== 'gameOver') {
        ...
        if (state.phase === 'playing') {
            ...
            if (state.phase === 'roundEnd') {
                ...
                state.devSeed = state.devSeed ?? SEED;
                roundCount++;
...
function assertNoHandsInPublicState() {
    const st = createInitialState();
    st.hands = { p1: [{ type: 'color', suit: 'green', value: 1 }] };
    const publicState = makePublicState(st);
    const handsVal = publicState.hands;
    if (handsVal !== undefined && handsVal !== null && typeof handsVal === 'object' && Object.keys(handsVal).length > 0) {
