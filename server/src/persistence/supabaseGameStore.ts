...
export interface PersistedGameContext {
  gameId: number;
...
export async function persistGameStart(roomCode: string, state: GameState): Promise<PersistedGameContext | null> {
  ...
  for (const p of activePlayers) {
    const { data, error } = await client
      ...
      .select('id')
      .single();
    if (error || !data) throw new Error(`persistGameStart players insert failed: ${error?.message ?? 'no data'}`);
    playerDbIds.set(p.playerId, data.id as number);
  }
  ...
  const gameId = gameData.id as number;
  ...
  const gamePlayersRows = state.activePlayerIds
    ...
    .filter((row): row is { game_id: number; player_id: number; seat_no: number; final_score: number } => !!row);

  if (gamePlayersRows.length > 0) {
    const { error: gpError } = await client.from('game_players').insert(gamePlayersRows);
    if (gpError) throw new Error(`persistGameStart game_players insert failed: ${gpError.message}`);
  }
...
export async function persistRoundEnd(args: {
  ...
  if (deleteError) throw new Error(`persistRoundEnd cleanup failed: ${deleteError.message}`);
  ...
  if (resultRows.length > 0) {
    ...
    if (resultError) throw new Error(`persistRoundEnd round_player_results insert failed: ${resultError.message}`);
  }
}
...
export async function persistGameOver(args: {
  ...
  for (const pid of state.activePlayerIds) {
    ...
    if (error) throw new Error(`persistGameOver game_players update failed: ${error.message}`);
  }
  ...
  if (gameError) throw new Error(`persistGameOver games update failed: ${gameError.message}`);
}
