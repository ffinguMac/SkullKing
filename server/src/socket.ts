...
export function setupSocketHandlers(io: Server): void {
  io.on('connection', (socket) => {
    ...
    socket.on('room:create', (data: { nickname: string }) => {
    ...
    socket.on('room:join', async (data: { roomCode: string; nickname: string; playerId?: string }) => {
    ...
    socket.on('room:leave', async (data: { roomCode: string; playerId: string }) => {
    ...
    socket.on('room:ready', async (data: { roomCode: string; playerId: string }) => {
      const roomCode = (data?.roomCode ?? currentRoomCode ?? '').toUpperCase();
      const pid = data?.playerId ?? playerId;
      const result = await dispatchAction(
        roomCode,
        { type: 'room:ready', roomCode, playerId: pid },
        pid,
        io
      );
      if (!result.success) emitError(result.error ?? 'READY_FAILED', result.error ?? '');
    });

    socket.on('game:start', async (data: { roomCode: string; playerId: string }) => {
      const roomCode = (data?.roomCode ?? currentRoomCode ?? '').toUpperCase();
      const pid = data?.playerId ?? playerId;
      const result = await dispatchAction(
        roomCode,
        { type: 'game:start', roomCode, playerId: pid },
        pid,
        io
      );
      if (!result.success) {
    ...
    socket.on('game:bet', async (data: { roomCode: string; playerId: string; value: number }) => {
      const roomCode = (data?.roomCode ?? currentRoomCode ?? '').toUpperCase();
      const pid = data?.playerId ?? playerId;
      const result = await dispatchAction(
        roomCode,
        { type: 'game:bet', roomCode, playerId: pid, value: data.value },
        pid,
        io
      );
      if (!result.success) emitError(result.error ?? 'BET_FAILED', result.error ?? '');
    });

    socket.on('game:playCard', async (data: { roomCode: string; playerId: string; cardId: number }) => {
      const roomCode = (data?.roomCode ?? currentRoomCode ?? '').toUpperCase();
      const pid = data?.playerId ?? playerId;
      const result = await dispatchAction(
        roomCode,
      ...
      if (!result.success) emitError(result.error ?? 'PLAY_FAILED', result.error ?? '');
    });

    socket.on('player:reconnect', async (data: { roomCode: string; playerId: string }) => {
      const roomCode = (data?.roomCode ?? '').toUpperCase();
